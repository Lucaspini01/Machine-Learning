...
def entrenar_red_con_validacion_scheduler(
    X_train, Y_train, X_val, Y_val, dims,
    epochs=50, lr_init=0.01, scheduler="none",
    lr_final=0.001, decay_rate=0.05, batch_size=5000
):
    parametros = inicializar_pesos(dims)
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, m, batch_size):
            ...
            # Actualizar parámetros en cada batch (SGD)
            parametros = actualizar_parametros(parametros, grads, lr)
            epoch_loss += loss
        ...
        print(f"Época {epoch+1}/{epochs} | lr={lr:.5f} | "
              f"Loss Train: {epoch_loss/num_batches:.4f} | Loss Val: {loss_val:.4f}")
